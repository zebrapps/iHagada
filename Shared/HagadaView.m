...
#import "HagadaView.h"
#import "HagadaCache.h"


#pragma mark -
#pragma mark Private interface

@interface HagadaView () 
...
- (void)setUpLayersForViewingMode {
    if (self.mode == HagadaViewModeSinglePage) {
...
- (void) initialize {
    ...
    if (self.bounds.size.width == 0 && self.bounds.size.height == 0)
        cachePageSize = CGSizeMake(100, 100);
    
    if (mode == HagadaViewModeFacingPages) {
        cachePageSize = CGSizeMake(self.bounds.size.width / 2.0f, self.bounds.size.height);
    }
	pageCache = [[HagadaCache alloc] initWithPageSize:cachePageSize];
	
	/* Sounds Init */
	
	//Get the filename of the sound file:
	NSString *pathForward = [NSString stringWithFormat:@"%@%@",
							 [[NSBundle mainBundle] resourcePath],
							 @"/page-flip-forward.wav"];
	
	//Get a URL for the sound file
	NSURL *filePathForward = [NSURL fileURLWithPath:pathForward isDirectory:NO];
	
	//Use audio sevices to create the sound
	AudioServicesCreateSystemSoundID((CFURLRef)filePathForward, &soundIDForward);	
	
	
	
	//Get the filename of the sound file:
	NSString *pathBackward = [NSString stringWithFormat:@"%@%@",
							  [[NSBundle mainBundle] resourcePath],
							  @"/page-flip-backward.wav"];
	
	//Get a URL for the sound file
	NSURL *filePathBackward = [NSURL fileURLWithPath:pathBackward isDirectory:NO];
	
	//Use audio sevices to create the sound
	AudioServicesCreateSystemSoundID((CFURLRef)filePathBackward, &soundIDBackward);		
}
...
- (void) setLayerFrames {
    ...
    if (self.mode == HagadaViewModeFacingPages) {
...
- (void) didTurnPageForward {
	...
	[self didTurnToPageAtIndex:currentPageIndex];
	[self playForwardSound];	
}

-(void) playForwardSound {
	//Use audio services to play the sound
	AudioServicesPlaySystemSound(soundIDForward);
}

-(void) playBackwardSound {
	//Use audio services to play the sound
	AudioServicesPlaySystemSound(soundIDBackward);
}
...
- (void) setDataSource:(id<HagadaViewDataSource>)value {
...
- (void) setCurrentPageIndex:(NSUInteger)aCurrentPageIndex {
	...
	if (self.mode == HagadaViewModeFacingPages && aCurrentPageIndex % 2 != 0) {
...
- (void) setMode:(HagadaViewMode)newMode
{
    mode = newMode;
    
    if (mode == HagadaViewModeSinglePage) {
        numberOfVisiblePages = 1;
