...
- (void) setLayerFrames {
	...
	topPage.frame = CGRectMake(leafEdge * rightPageBoundsRect.size.width, 
							   rightPageBoundsRect.origin.y, 
							   rightPageBoundsRect.size.width - (leafEdge * rightPageBoundsRect.size.width), 
							   rightPageBoundsRect.size.height);
	
	topPageReverse.frame = CGRectMake(rightPageBoundsRect.origin.x + (leafEdge * rightPageBoundsRect.size.width),
									  rightPageBoundsRect.origin.y, 
...
- (void) setLeafEdge:(CGFloat)aLeafEdge {
	...
	/* Yuval - Due to the hebrew changes, now the leafEdge changed sizes so the opacity need to be as the old function */
	if (leafEdge == 0) {
		pageOpacity = 0;
	} else if (leafEdge == 1) {
		pageOpacity = 1.0;
...
- (void) layoutSubviews {	
	...
	if (!CGSizeEqualToSize(pageSize, desiredPageSize)) {
		...
		/* Yuval The next page will be from the left side and prev page from the right */
